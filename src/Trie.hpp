#ifndef CPPJB_TRIE_HPP
#define CPPJB_TRIE_HPP
...
// for debugging
inline std::ostream & operator << (std::ostream& os, const DictUnit& unit) {
  std::string s;
  s << unit.word;
  return os << limonp::StringFormat("%s %s %.3lf", s.c_str(), unit.tag.c_str(), unit.weight);
}
